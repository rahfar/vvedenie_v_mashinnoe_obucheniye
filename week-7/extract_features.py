...
def extract_match_features(match, time_point=None):
        ...
        feats += [
            ...
            (
                ...
                max(
                    ...
                    + [
                        entry["level"]
                        for entry in filter_events(
                            player["ability_upgrades"], time_point
                        )
                    ]
                ),
            ),
                ...
                len(
                    [
                        ...
                        for event in filter_events(
                            other_player["kills_log"], time_point
                        )
            ...
            (
                player_id + "_items",
                len(filter_events(player["purchase_log"], time_point)),
            ),
        ]

    # first blood
    first_blood_objectives = filter_events(
        [obj for obj in match["objectives"] if obj["type"] == "firstblood"], time_point
    )
    ...
    feats += [
        ...
        (
            "first_blood_team",
            int(fb["player1"] >= 5) if fb.get("player1") is not None else None,
        ),
    ...
    for team, team_players in (("radiant", radiant_players), ("dire", dire_players)):
        for item_id, item_name in extract_items_time:
            ...
            feats += [("%s_%s_time" % (team, item_name), first_item_time)]
        ...
        for item_id, item_name in extract_items_count:
            ...
            feats += [("%s_%s_count" % (team, item_name), item_count)]
        ...
        feats += [
            (
                ...
                min([entry["time"] for entry in team_wards])
                if len(team_wards) > 0
                else None,
            ),
        ]
...
def create_table(matches_filename, time_point):
    ...
    for match in iterate_matches(matches_filename):
        ...
        for key, value in features.iteritems():
            df[key].append(value)
    df = (
        pandas.DataFrame.from_records(df)
        .ix[:, fields]
        .set_index("match_id")
        .sort_index()
    )
